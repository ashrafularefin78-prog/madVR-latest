...
if /i "%~nx0"=="activate debug mode.bat" (title activate madVR debug mode...)
if /i "%~nx0"=="activate release mode.bat" (title activate madVR release mode...)
...
if /i "%~nx0"=="activate debug mode.bat" (
    echo    file "madVR [debug].ax" not found
) else if /i "%~nx0"=="activate release mode.bat" (
    echo    file "madVR [release].ax" not found
) else (
...
if exist "madVR [debug].ax" (
    echo    release mode was successfully activated
) else (
    echo    activating release mode failed
)
echo.
pause >nul
...
if exist "madVR [release].ax" (
    echo    debug mode was successfully activated
) else (
    echo    activating debug mode failed
)
echo.
pause >nul
