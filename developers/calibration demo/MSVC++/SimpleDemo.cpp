...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
  if (!madVR_IsAvailable())
    MessageBoxA(0, "madVR network dll not found.", "Error", MB_ICONERROR);
  else
    if (madVR_Connect())
    {
      if (madVR_IsLocal())
      ...
      for (int i1 = 0; i1 < _countof(GrayRamp); i1++)
      {
        ...
        {
          MessageBoxA(0, "Test pattern failure.", "Error", MB_ICONERROR);
          break;
        }
      ...
      MessageBoxA(0, "No madTPG instance found.", "Error", MB_ICONERROR);
