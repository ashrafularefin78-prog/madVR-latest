// ***************************************************************
//  madTPG.cpp                version: 1.6.0  ·  date: 2017-09-12
//  -------------------------------------------------------------
...
{
  return Init() && (Enable3dlut) && Enable3dlut();
}

BOOL madVR_Disable3dlut()
{
  return Init() && (Disable3dlut) && Disable3dlut();
}
...
{
  return Init() && (SetDeviceGammaRamp_) && SetDeviceGammaRamp_(ramp);
}

BOOL madVR_SetOsdText(LPCWSTR text)
{
  return Init() && (SetOsdText) && SetOsdText(text);
}
...
{
  return Init() && (ShowProgressBar) && ShowProgressBar(numberOfRgbMeasurements);
}
...
{
  return Init() && (ShowRGB) && ShowRGB(r, g, b);
}
...
{
  return Init() && (Disconnect) && Disconnect();
}
...
{
  return Init() && (Find_Async) && Find_Async(window, msg);
}

BOOL madVR_ConnectToInstance(HANDLE handle, ULONGLONG instance)
{
  return Init() && (ConnectToInstance) && ConnectToInstance(handle, instance);
}
